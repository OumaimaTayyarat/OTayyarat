...
import room_model_file from "url:../../public/models/room2.glb"
...
import Avatar_model_file from "url:../../public/animations/setupoumaima3.fbx"
...
const T0_LOAD = {
  models: [
    {id: "Avatar", file: Avatar_model_file, scale: 0.001, position: [-0.1,  -0.23, 0 ]},
      { id: "room", file: room_model_file, scale: 0.15, position: [0.03, -0.26, -0.55] },
    { id: "service", file: service_model_file, scale: 0.0000001, position: [0, 0, 0] },
  ...
  loadModels() {
    T0_LOAD.models.forEach((item) => {
        if (item.id === "Avatar") {
            ...
            // Chargement du modèle FBX principal
            const fbxLoader = new FBXLoader(this.manager);
            fbxLoader.load(
                item.file,
                (loadedFbx) => {
                    fbx = loadedFbx; // Affecter loadedFbx à fbx
                    if (item.position) fbx.position.set(...item.position);
                    if (item.scale) fbx.scale.set(item.scale, item.scale, item.scale);

                    // Configuration des ombres pour les objets du modèle
                    fbx.traverse(c => {
                        c.castShadow = true;
                    });

                    // Enregistrement du modèle dans les actifs
                    this.assets.models[item.id] = fbx;

                    // Chargement des animations du modèle
                    const animations = fbx.animations;
                    
                    // Création du mélangeur (AnimationMixer) et ajout des animations
                    this.mixer = new AnimationMixer(fbx);
                    // this.assets.mixers[item.id] = this.mixer; // Stockage du mélangeur
                    ...
                    this.scene.add(fbx); // Ajout du modèle à la scène

                    // Incrémentation du compteur de chargement des modèles
                    this.modelLoadCount++;
                    this.checkModels(); // Vérification de l'achèvement du chargement des modèles
                }
            );
        } else {
    ...
    this.loadNext(); // Passage à l'étape suivante du chargement
}
